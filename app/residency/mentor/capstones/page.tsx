...
export default function MentorCapstonesPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (residentIds.length === 0) { setLoading(false); return }
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr', gap: '1.5rem' }}>
          <div>
            ...
            {selected.reviews?.length > 0 && (
              <div>
                <h2 style={{ fontSize: '0.875rem', fontWeight: 600, color: 'var(--r-text-muted)', marginBottom: '0.5rem' }}>Previous Reviews</h2>
                {selected.reviews.map((r: any) => (
                  <div key={r.id} className="r-card" style={{ padding: '1rem', marginBottom: '0.5rem' }}>
                    <p style={{ fontSize: '0.8125rem', fontWeight: 600, marginBottom: '0.25rem' }}>
                      ...
                      <span style={{ marginLeft: '0.5rem', fontSize: '0.625rem', color: r.verdict === 'approved' ? '#2d6a4f' : '#991b1b' }}>
                        {r.verdict === 'approved' ? 'APPROVED' : 'REVISION NEEDED'}
                      </span>
                    </p>
                    {r.strengths && <p style={{ fontSize: '0.8125rem', lineHeight: 1.6 }}>Strengths: {r.strengths}</p>}
                    {r.growth_areas && <p style={{ fontSize: '0.8125rem', lineHeight: 1.6 }}>Growth: {r.growth_areas}</p>}
                  </div>
                ))}
              </div>
            )}
          ...
          <div className="r-card" style={{ padding: '1.5rem', alignSelf: 'start' }}>
            ...
            <div style={{ marginBottom: '1rem' }}>
              ...
              <textarea className="r-textarea" rows={2} value={reviewForm.practicum_connection}
                ...
                placeholder="Connection to practicum experience..." />
            </div>
            ...
            <button className="r-btn r-btn-primary" onClick={submitReview} disabled={submittingReview}
              ...
              {submittingReview ? 'Submitting...' : reviewForm.verdict === 'approved' ? 'Approve' : 'Request Revision'}
            </button>
          </div>
        </div>
      </div>
  ...
  return (
    <div>
      ...
      <p style={{ color: 'var(--r-text-muted)', fontSize: '0.875rem', marginBottom: '1.5rem' }}>
        Review capstone projects from your assigned residents.
      </p>
          ...
          {reviewed.map(c => {
            ...
            return (
              <div key={c.id} className="r-card" onClick={() => setSelected(c)}
                style={{ padding: '1rem', marginBottom: '0.5rem', cursor: 'pointer', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                  <p style={{ fontWeight: 600, fontSize: '0.9375rem' }}>{res?.profile?.first_name} {res?.profile?.last_name}</p>
                  <p style={{ fontSize: '0.8125rem', color: 'var(--r-text-muted)' }}>{c.lesson_title || 'Untitled'}</p>
                </div>
                <span style={{
      ...
      {capstones.length === 0 && (
        <EmptyState title="No capstones" message="Capstone submissions from your residents will appear here." />
      )}
    </div>
  )
}
